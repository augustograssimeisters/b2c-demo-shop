 ...
 *
 *
 * @package    propel.generator.src.Orm.Zed.Wishlist.Persistence.Base
 */
abstract class SpyWishlist implements ActiveRecordInterface
{
    /**
     * TableMap class name
     *
     * @var string
     */
    public const TABLE_MAP = '\\Orm\\Zed\\Wishlist\\Persistence\\Map\\SpyWishlistTableMap';
     ...
     * @phpstan-var ObjectCollection&\Traversable<ChildSpyWishlistItem> Collection to store aggregation of ChildSpyWishlistItem objects.
     */
     ...
     * @phpstan-var ObjectCollection&\Traversable<ChildSpyWishlistItem>
     */
    protected $spyWishlistItemsScheduledForDeletion = null;

    /**
     * Initializes internal state of Orm\Zed\Wishlist\Persistence\Base\SpyWishlist object.
     */
    ...
    public function setIdWishlist($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->id_wishlist !== $v) {
            $this->id_wishlist = $v;
    ...
    public function setName($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[SpyWishlistTableMap::COL_NAME] = true;
        }
    ...
    public function setCreatedAt($v)
    {
        ...
        if ($this->created_at !== null || $dt !== null) {
            if ($this->created_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->created_at->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[SpyWishlistTableMap::COL_CREATED_AT] = true;
            }
    ...
    public function setUpdatedAt($v)
    {
        ...
        if ($this->updated_at !== null || $dt !== null) {
            if ($this->updated_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->updated_at->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[SpyWishlistTableMap::COL_UPDATED_AT] = true;
            }
        } // if either are not null
    ...
    public function setUuid($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->uuid !== $v) {
            $this->uuid = $v;
            $this->modifiedColumns[SpyWishlistTableMap::COL_UUID] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SpyWishlistTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SpyWishlistTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SpyWishlistTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->updated_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : SpyWishlistTableMap::translateFieldName('Uuid', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 6; // 6 = SpyWishlistTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Orm\\Zed\\Wishlist\\Persistence\\SpyWishlist'), 0, $e);
        }
    }
    ...
    public function save(?ConnectionInterface $con = null): int
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($isInsert) {
                ...
                if (!$this->isColumnModified(SpyWishlistTableMap::COL_CREATED_AT)) {
                    $this->setCreatedAt($highPrecision);
                }
                if (!$this->isColumnModified(SpyWishlistTableMap::COL_UPDATED_AT)) {
                    $this->setUpdatedAt($highPrecision);
                }
            } else {
                $ret = $ret && $this->preUpdate($con);
                // timestampable behavior
                if ($this->isModified() && !$this->isColumnModified(SpyWishlistTableMap::COL_UPDATED_AT)) {
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(SpyWishlistTableMap::COL_NAME)) {
        ...
        if ($this->isColumnModified(SpyWishlistTableMap::COL_UUID)) {
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(SpyWishlistTableMap::COL_NAME)) {
            $criteria->add(SpyWishlistTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(SpyWishlistTableMap::COL_CREATED_AT)) {
            $criteria->add(SpyWishlistTableMap::COL_CREATED_AT, $this->created_at);
        }
        if ($this->isColumnModified(SpyWishlistTableMap::COL_UPDATED_AT)) {
            $criteria->add(SpyWishlistTableMap::COL_UPDATED_AT, $this->updated_at);
        }
        if ($this->isColumnModified(SpyWishlistTableMap::COL_UUID)) {
            $criteria->add(SpyWishlistTableMap::COL_UUID, $this->uuid);
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Orm\Zed\Wishlist\Persistence\SpyWishlist (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Orm\Zed\Wishlist\Persistence\SpyWishlist Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildSpyWishlistItem> List of ChildSpyWishlistItem objects
     * @throws \Propel\Runtime\Exception\PropelException
     */
     ...
     */
    protected function doAddSpyWishlistItem(ChildSpyWishlistItem $spyWishlistItem): void
    {
    ...
    protected function setGeneratedUuid()
    {
        ...
        $name = 'spy_wishlist' . '.' . $this->getFkCustomer() . '.' . $this->getIdWishlist();
