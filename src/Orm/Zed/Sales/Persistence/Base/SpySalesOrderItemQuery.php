 ...
 *
 * @method     ChildSpySalesOrderItemQuery leftJoinSpySalesOrderItemGiftCard($relationAlias = null) Adds a LEFT JOIN clause to the query using the SpySalesOrderItemGiftCard relation
 * @method     ChildSpySalesOrderItemQuery rightJoinSpySalesOrderItemGiftCard($relationAlias = null) Adds a RIGHT JOIN clause to the query using the SpySalesOrderItemGiftCard relation
 * @method     ChildSpySalesOrderItemQuery innerJoinSpySalesOrderItemGiftCard($relationAlias = null) Adds a INNER JOIN clause to the query using the SpySalesOrderItemGiftCard relation
 *
 * @method     ChildSpySalesOrderItemQuery joinWithSpySalesOrderItemGiftCard($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the SpySalesOrderItemGiftCard relation
 *
 * @method     ChildSpySalesOrderItemQuery leftJoinWithSpySalesOrderItemGiftCard() Adds a LEFT JOIN clause and with to the query using the SpySalesOrderItemGiftCard relation
 * @method     ChildSpySalesOrderItemQuery rightJoinWithSpySalesOrderItemGiftCard() Adds a RIGHT JOIN clause and with to the query using the SpySalesOrderItemGiftCard relation
 * @method     ChildSpySalesOrderItemQuery innerJoinWithSpySalesOrderItemGiftCard() Adds a INNER JOIN clause and with to the query using the SpySalesOrderItemGiftCard relation
 *
...
abstract class SpySalesOrderItemQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCard object
     *
     * @param \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCard|ObjectCollection $spySalesOrderItemGiftCard the related object to use as filter
     * @param string|null $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this The current query, for fluid interface
     */
    public function filterBySpySalesOrderItemGiftCard($spySalesOrderItemGiftCard, ?string $comparison = null)
    {
        if ($spySalesOrderItemGiftCard instanceof \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCard) {
            $this
                ->addUsingAlias(SpySalesOrderItemTableMap::COL_ID_SALES_ORDER_ITEM, $spySalesOrderItemGiftCard->getFkSalesOrderItem(), $comparison);

            return $this;
        } elseif ($spySalesOrderItemGiftCard instanceof ObjectCollection) {
            $this
                ->useSpySalesOrderItemGiftCardQuery()
                ->filterByPrimaryKeys($spySalesOrderItemGiftCard->getPrimaryKeys())
                ->endUse();

            return $this;
        } else {
            throw new PropelException('filterBySpySalesOrderItemGiftCard() only accepts arguments of type \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCard or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the SpySalesOrderItemGiftCard relation
     *
     * @param string|null $relationAlias Optional alias for the relation
     * @param string|null $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return $this The current query, for fluid interface
     */
    public function joinSpySalesOrderItemGiftCard(?string $relationAlias = null, ?string $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('SpySalesOrderItemGiftCard');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'SpySalesOrderItemGiftCard');
        }
    ...
    /**
     * Use the SpySalesOrderItemGiftCard relation SpySalesOrderItemGiftCard object
     *
     * @see useQuery()
     *
     * @param string $relationAlias optional alias for the relation,
     ...
     * @param string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery A secondary query class using the current class as primary query
     */
    public function useSpySalesOrderItemGiftCardQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinSpySalesOrderItemGiftCard($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'SpySalesOrderItemGiftCard', '\Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery');
    }

    /**
     * Use the SpySalesOrderItemGiftCard relation SpySalesOrderItemGiftCard object
     *
     * @param callable(\Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery):\Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery $callable A function working on the related query
     *
     * @param string|null $relationAlias optional alias for the relation
     *
     * @param string|null $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     ...
     * @return $this
     */
    public function withSpySalesOrderItemGiftCardQuery(
        callable $callable,
        string $relationAlias = null,
        ?string $joinType = Criteria::INNER_JOIN
    ) {
        $relatedQuery = $this->useSpySalesOrderItemGiftCardQuery(
            $relationAlias,
            $joinType
        );
        $callable($relatedQuery);
    ...
    /**
     * Use the relation to SpySalesOrderItemGiftCard table for an EXISTS query.
     *
     * @see \Propel\Runtime\ActiveQuery\ModelCriteria::useExistsQuery()
     *
     * @param string|null $modelAlias sets an alias for the nested query
     ...
     * @param string $typeOfExists Either ExistsQueryCriterion::TYPE_EXISTS or ExistsQueryCriterion::TYPE_NOT_EXISTS
     *
     * @return \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery The inner query object of the EXISTS statement
     */
    public function useSpySalesOrderItemGiftCardExistsQuery($modelAlias = null, $queryClass = null, $typeOfExists = 'EXISTS')
    {
        /** @var $q \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery */
        $q = $this->useExistsQuery('SpySalesOrderItemGiftCard', $modelAlias, $queryClass, $typeOfExists);
        return $q;
    }

    /**
     * Use the relation to SpySalesOrderItemGiftCard table for a NOT EXISTS query.
     *
     * @see useSpySalesOrderItemGiftCardExistsQuery()
     *
     * @param string|null $modelAlias sets an alias for the nested query
     * @param string|null $queryClass Allows to use a custom query class for the exists query, like ExtendedBookQuery::class
     *
     * @return \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery The inner query object of the NOT EXISTS statement
     */
    public function useSpySalesOrderItemGiftCardNotExistsQuery($modelAlias = null, $queryClass = null)
    {
        /** @var $q \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery */
        $q = $this->useExistsQuery('SpySalesOrderItemGiftCard', $modelAlias, $queryClass, 'NOT EXISTS');
        return $q;
    }

    /**
     * Use the relation to SpySalesOrderItemGiftCard table for an IN query.
     *
     * @see \Propel\Runtime\ActiveQuery\ModelCriteria::useInQuery()
     *
     * @param string|null $modelAlias sets an alias for the nested query
     ...
     * @param string $typeOfIn Criteria::IN or Criteria::NOT_IN
     *
     * @return \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery The inner query object of the IN statement
     */
    public function useInSpySalesOrderItemGiftCardQuery($modelAlias = null, $queryClass = null, $typeOfIn = 'IN')
    {
        /** @var $q \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery */
        $q = $this->useInQuery('SpySalesOrderItemGiftCard', $modelAlias, $queryClass, $typeOfIn);
        return $q;
    }

    /**
     * Use the relation to SpySalesOrderItemGiftCard table for a NOT IN query.
     *
     * @see useSpySalesOrderItemGiftCardInQuery()
     *
     * @param string|null $modelAlias sets an alias for the nested query
     * @param string|null $queryClass Allows to use a custom query class for the NOT IN query, like ExtendedBookQuery::class
     *
     * @return \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery The inner query object of the NOT IN statement
     */
    public function useNotInSpySalesOrderItemGiftCardQuery($modelAlias = null, $queryClass = null)
    {
        /** @var $q \Orm\Zed\Sales\Persistence\SpySalesOrderItemGiftCardQuery */
        $q = $this->useInQuery('SpySalesOrderItemGiftCard', $modelAlias, $queryClass, 'NOT IN');
        return $q;
    }
