 ...
 *
 * @method     \Orm\Zed\ConfigurableBundle\Persistence\SpyConfigurableBundleTemplateSlotQuery|\Orm\Zed\ProductList\Persistence\SpyProductListProductConcreteQuery|\Orm\Zed\ProductList\Persistence\SpyProductListCategoryQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class SpyProductListQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id_product_list`, `key`, `title`, `type`, `created_at`, `updated_at` FROM `spy_product_list` WHERE `id_product_list` = :p0';
        try {
