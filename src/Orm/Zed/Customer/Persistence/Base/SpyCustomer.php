...
abstract class SpyCustomer implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|SpyWishlist[] Collection to store aggregation of SpyWishlist objects.
     * @phpstan-var ObjectCollection&\Traversable<SpyWishlist> Collection to store aggregation of SpyWishlist objects.
     */
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|SpyWishlist[]
     * @phpstan-var ObjectCollection&\Traversable<SpyWishlist>
     */
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->spyWishlistsScheduledForDeletion !== null) {
                if (!$this->spyWishlistsScheduledForDeletion->isEmpty()) {
                    \Orm\Zed\Wishlist\Persistence\SpyWishlistQuery::create()
     ...
     * @return void
     */
    public function resetPartialSpyWishlists($v = true): void
    {
     ...
     * @return void
     */
    public function initSpyWishlists(bool $overrideExisting = true): void
    {
        if (null !== $this->collSpyWishlists && !$overrideExisting) {
            return;
        }

        $collectionClassName = SpyWishlistTableMap::getTableMap()->getCollectionClassName();

        $this->collSpyWishlists = new $collectionClassName;
        $this->collSpyWishlists->setModel('\Orm\Zed\Wishlist\Persistence\SpyWishlist');
    }

    /**
     * Gets an array of SpyWishlist objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param ConnectionInterface $con optional connection object
     * @return ObjectCollection|SpyWishlist[] List of SpyWishlist objects
     * @phpstan-return ObjectCollection&\Traversable<SpyWishlist> List of SpyWishlist objects
     * @throws \Propel\Runtime\Exception\PropelException
     */
        ...
        if (null === $this->collSpyWishlists || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collSpyWishlists) {
                    ...
                    $collSpyWishlists = new $collectionClassName;
                    $collSpyWishlists->setModel('\Orm\Zed\Wishlist\Persistence\SpyWishlist');

                    return $collSpyWishlists;
                }
            } else {
                $collSpyWishlists = SpyWishlistQuery::create(null, $criteria)
                    ->filterBySpyCustomer($this)
                    ->find($con);
    ...
    /**
     * Sets a collection of SpyWishlist objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
    ...
    {
        /** @var SpyWishlist[] $spyWishlistsToDelete */
    ...
    /**
     * Returns the number of related BaseSpyWishlist objects.
     *
     * @param Criteria $criteria
     * @param bool $distinct
     * @param ConnectionInterface $con
     * @return int Count of related BaseSpyWishlist objects.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function countSpyWishlists(?Criteria $criteria = null, bool $distinct = false, ?ConnectionInterface $con = null): int
    {
        $partial = $this->collSpyWishlistsPartial && !$this->isNew();
        if (null === $this->collSpyWishlists || null !== $criteria || $partial) {
            ...
            $query = SpyWishlistQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
    ...
    /**
     * Method called to associate a SpyWishlist object to this object
     * through the SpyWishlist foreign key attribute.
     *
     * @param SpyWishlist $l SpyWishlist
     * @return $this The current object (for fluent API support)
     */
    public function addSpyWishlist(SpyWishlist $l)
    {
        if ($this->collSpyWishlists === null) {
            $this->initSpyWishlists();
            $this->collSpyWishlistsPartial = true;
        ...
        if (!$this->collSpyWishlists->contains($l)) {
            ...
            if ($this->spyWishlistsScheduledForDeletion and $this->spyWishlistsScheduledForDeletion->contains($l)) {
                $this->spyWishlistsScheduledForDeletion->remove($this->spyWishlistsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param SpyWishlist $spyWishlist The SpyWishlist object to add.
     */
    protected function doAddSpyWishlist(SpyWishlist $spyWishlist): void
    {
    ...
    /**
     * @param SpyWishlist $spyWishlist The SpyWishlist object to remove.
     * @return $this The current object (for fluent API support)
     */
    public function removeSpyWishlist(SpyWishlist $spyWishlist)
    {
