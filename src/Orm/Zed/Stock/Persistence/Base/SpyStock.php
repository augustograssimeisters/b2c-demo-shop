...
use Orm\Zed\PickingList\Persistence\Base\SpyPickingList as BaseSpyPickingList;
...
use Orm\Zed\WarehouseUser\Persistence\Base\SpyWarehouseUserAssignment as BaseSpyWarehouseUserAssignment;
...
abstract class SpyStock implements ActiveRecordInterface
{
    ...
    /**
     * The value for the picking_list_strategy field.
     *
     * @var        string|null
     */
    protected $picking_list_strategy;
    ...
    /**
     * @var        ObjectCollection|SpyPickingList[] Collection to store aggregation of SpyPickingList objects.
     * @phpstan-var ObjectCollection&\Traversable<SpyPickingList> Collection to store aggregation of SpyPickingList objects.
     */
    protected $collSpyPickingLists;
    protected $collSpyPickingListsPartial;
    ...
    /**
     * @var        ObjectCollection|SpyWarehouseUserAssignment[] Collection to store aggregation of SpyWarehouseUserAssignment objects.
     * @phpstan-var ObjectCollection&\Traversable<SpyWarehouseUserAssignment> Collection to store aggregation of SpyWarehouseUserAssignment objects.
     */
    protected $collSpyWarehouseUserAssignments;
    protected $collSpyWarehouseUserAssignmentsPartial;
     ...
     * @var ObjectCollection|SpyPickingList[]
     * @phpstan-var ObjectCollection&\Traversable<SpyPickingList>
     */
    protected $spyPickingListsScheduledForDeletion = null;
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|SpyWarehouseUserAssignment[]
     * @phpstan-var ObjectCollection&\Traversable<SpyWarehouseUserAssignment>
     */
    protected $spyWarehouseUserAssignmentsScheduledForDeletion = null;
    ...
    /**
     * Get the [picking_list_strategy] column value.
     *
     * @return string|null
     */
    public function getPickingListStrategy()
    {
        return $this->picking_list_strategy;
    }
    ...
    /**
     * Set the value of [picking_list_strategy] column.
     *
     * @param string|null $v New value
     * @return $this The current object (for fluent API support)
     */
    public function setPickingListStrategy($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->picking_list_strategy !== $v) {
            $this->picking_list_strategy = $v;
            $this->modifiedColumns[SpyStockTableMap::COL_PICKING_LIST_STRATEGY] = true;
        }
    ...
    public function setUuid($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->uuid !== $v) {
            $this->uuid = $v;
            $this->modifiedColumns[SpyStockTableMap::COL_UUID] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SpyStockTableMap::translateFieldName('PickingListStrategy', TableMap::TYPE_PHPNAME, $indexType)];
            $this->picking_list_strategy = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SpyStockTableMap::translateFieldName('Uuid', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = SpyStockTableMap::NUM_HYDRATE_COLUMNS.
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collSpyWarehouseUserAssignments = null;
    ...
    protected function doSave(ConnectionInterface $con): int
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->spyPickingListsScheduledForDeletion !== null) {
                if (!$this->spyPickingListsScheduledForDeletion->isEmpty()) {
                    \Orm\Zed\PickingList\Persistence\SpyPickingListQuery::create()
                        ->filterByPrimaryKeys($this->spyPickingListsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->spyPickingListsScheduledForDeletion = null;
                }
            }

            if ($this->collSpyPickingLists !== null) {
                foreach ($this->collSpyPickingLists as $referrerFK) {
            ...
            if ($this->spyWarehouseUserAssignmentsScheduledForDeletion !== null) {
                if (!$this->spyWarehouseUserAssignmentsScheduledForDeletion->isEmpty()) {
                    \Orm\Zed\WarehouseUser\Persistence\SpyWarehouseUserAssignmentQuery::create()
                        ->filterByPrimaryKeys($this->spyWarehouseUserAssignmentsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->spyWarehouseUserAssignmentsScheduledForDeletion = null;
                }
            }

            if ($this->collSpyWarehouseUserAssignments !== null) {
                foreach ($this->collSpyWarehouseUserAssignments as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(SpyStockTableMap::COL_PICKING_LIST_STRATEGY)) {
            $modifiedColumns[':p' . $index++]  = 'picking_list_strategy';
        }
        if ($this->isColumnModified(SpyStockTableMap::COL_UUID)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'name':
                        ...
                        break;
                    case 'picking_list_strategy':
                        $stmt->bindValue($identifier, $this->picking_list_strategy, PDO::PARAM_STR);
    ...
    public function getByPosition(int $pos)
    {
        switch ($pos) {
            ...
            case 3:
                return $this->getPickingListStrategy();
    ...
    public function toArray(string $keyType = TableMap::TYPE_FIELDNAME, bool $includeLazyLoadColumns = true, array $alreadyDumpedObjects = [], bool $includeForeignObjects = false): array
    {
        ...
        $result = [
            ...
            $keys[2] => $this->getName(),
            $keys[3] => $this->getPickingListStrategy(),
            $keys[4] => $this->getUuid(),
        ];
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collSpyPickingLists) {
                ...
                switch ($keyType) {
                    ...
                    case TableMap::TYPE_FIELDNAME:
                        $key = 'spy_picking_lists';
                        break;
                    default:
                        $key = 'SpyPickingLists';
                }

                $result[$key] = $this->collSpyPickingLists->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
            ...
            if (null !== $this->collSpyWarehouseUserAssignments) {

                switch ($keyType) {
                    case TableMap::TYPE_CAMELNAME:
                        $key = 'spyWarehouseUserAssignments';
                        break;
                    case TableMap::TYPE_FIELDNAME:
                        $key = 'spy_warehouse_user_assignments';
                        break;
                    default:
                        $key = 'SpyWarehouseUserAssignments';
                }

                $result[$key] = $this->collSpyWarehouseUserAssignments->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function setByPosition(int $pos, $value)
    {
        switch ($pos) {
            ...
            case 2:
                ...
                break;
            case 3:
                $this->setPickingListStrategy($value);
                break;
            case 4:
                $this->setUuid($value);
                break;
    ...
    public function fromArray(array $arr, string $keyType = TableMap::TYPE_FIELDNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) {
            $this->setPickingListStrategy($arr[$keys[3]]);
        }
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(SpyStockTableMap::COL_PICKING_LIST_STRATEGY)) {
            $criteria->add(SpyStockTableMap::COL_PICKING_LIST_STRATEGY, $this->picking_list_strategy);
        }
        if ($this->isColumnModified(SpyStockTableMap::COL_UUID)) {
            $criteria->add(SpyStockTableMap::COL_UUID, $this->uuid);
        }
    ...
    public function copyInto(object $copyObj, bool $deepCopy = false, bool $makeNew = true): void
    {
        ...
        $copyObj->setPickingListStrategy($this->getPickingListStrategy());
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getSpyPickingLists() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addSpyPickingList($relObj->copy($deepCopy));
                }
            }
            ...
            foreach ($this->getSpyWarehouseUserAssignments() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addSpyWarehouseUserAssignment($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('SpyPickingList' === $relationName) {
            $this->initSpyPickingLists();
            return;
        }
        ...
        if ('SpyWarehouseUserAssignment' === $relationName) {
            $this->initSpyWarehouseUserAssignments();
            return;
        }
    }

    /**
     * Clears out the collSpyPickingLists collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return $this
     * @see addSpyPickingLists()
     */
    public function clearSpyPickingLists()
    {
        $this->collSpyPickingLists = null; // important to set this to NULL since that means it is uninitialized

        return $this;
    }

    /**
     * Reset is the collSpyPickingLists collection loaded partially.
     *
     * @return void
     */
    public function resetPartialSpyPickingLists($v = true): void
    {
        $this->collSpyPickingListsPartial = $v;
    }

    /**
     * Initializes the collSpyPickingLists collection.
     *
     * By default this just sets the collSpyPickingLists collection to an empty array (like clearcollSpyPickingLists());
     ...
     * @return void
     */
    public function initSpyPickingLists(bool $overrideExisting = true): void
    {
        if (null !== $this->collSpyPickingLists && !$overrideExisting) {
            return;
        }

        $collectionClassName = SpyPickingListTableMap::getTableMap()->getCollectionClassName();

        $this->collSpyPickingLists = new $collectionClassName;
        $this->collSpyPickingLists->setModel('\Orm\Zed\PickingList\Persistence\SpyPickingList');
    }

    /**
     * Gets an array of SpyPickingList objects which contain a foreign key that references this object.
     *
     ...
     * @return ObjectCollection|SpyPickingList[] List of SpyPickingList objects
     * @phpstan-return ObjectCollection&\Traversable<SpyPickingList> List of SpyPickingList objects
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function getSpyPickingLists(?Criteria $criteria = null, ?ConnectionInterface $con = null)
    {
        $partial = $this->collSpyPickingListsPartial && !$this->isNew();
        if (null === $this->collSpyPickingLists || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collSpyPickingLists) {
                    ...
                    $collSpyPickingLists = new $collectionClassName;
                    $collSpyPickingLists->setModel('\Orm\Zed\PickingList\Persistence\SpyPickingList');

                    return $collSpyPickingLists;
                }
            } else {
                $collSpyPickingLists = SpyPickingListQuery::create(null, $criteria)
                    ->filterBySpyStock($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collSpyPickingListsPartial && count($collSpyPickingLists)) {
                        $this->initSpyPickingLists(false);

                        foreach ($collSpyPickingLists as $obj) {
                            if (false == $this->collSpyPickingLists->contains($obj)) {
                                $this->collSpyPickingLists->append($obj);
                            }
                        }

                        $this->collSpyPickingListsPartial = true;
                    }

                    return $collSpyPickingLists;
                }

                if ($partial && $this->collSpyPickingLists) {
                    foreach ($this->collSpyPickingLists as $obj) {
                        if ($obj->isNew()) {
                            $collSpyPickingLists[] = $obj;
                        }
                    }
                }

                $this->collSpyPickingLists = $collSpyPickingLists;
                $this->collSpyPickingListsPartial = false;
            }
        }

        return $this->collSpyPickingLists;
    }

    /**
     * Sets a collection of SpyPickingList objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param Collection $spyPickingLists A Propel collection.
     * @param ConnectionInterface $con Optional connection object
     * @return $this The current object (for fluent API support)
     */
    public function setSpyPickingLists(Collection $spyPickingLists, ?ConnectionInterface $con = null)
    {
        /** @var SpyPickingList[] $spyPickingListsToDelete */
        $spyPickingListsToDelete = $this->getSpyPickingLists(new Criteria(), $con)->diff($spyPickingLists);


        $this->spyPickingListsScheduledForDeletion = $spyPickingListsToDelete;

        foreach ($spyPickingListsToDelete as $spyPickingListRemoved) {
            $spyPickingListRemoved->setSpyStock(null);
        }

        $this->collSpyPickingLists = null;
        foreach ($spyPickingLists as $spyPickingList) {
            $this->addSpyPickingList($spyPickingList);
        }

        $this->collSpyPickingLists = $spyPickingLists;
        $this->collSpyPickingListsPartial = false;
    ...
    /**
     * Returns the number of related BaseSpyPickingList objects.
     *
     * @param Criteria $criteria
     * @param bool $distinct
     * @param ConnectionInterface $con
     * @return int Count of related BaseSpyPickingList objects.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function countSpyPickingLists(?Criteria $criteria = null, bool $distinct = false, ?ConnectionInterface $con = null): int
    {
        $partial = $this->collSpyPickingListsPartial && !$this->isNew();
        if (null === $this->collSpyPickingLists || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collSpyPickingLists) {
                return 0;
            }

            if ($partial && !$criteria) {
                return count($this->getSpyPickingLists());
            }

            $query = SpyPickingListQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collSpyPickingLists);
    }

    /**
     * Method called to associate a SpyPickingList object to this object
     * through the SpyPickingList foreign key attribute.
     *
     * @param SpyPickingList $l SpyPickingList
     * @return $this The current object (for fluent API support)
     */
    public function addSpyPickingList(SpyPickingList $l)
    {
        if ($this->collSpyPickingLists === null) {
            $this->initSpyPickingLists();
            $this->collSpyPickingListsPartial = true;
        ...
        if (!$this->collSpyPickingLists->contains($l)) {
            $this->doAddSpyPickingList($l);

            if ($this->spyPickingListsScheduledForDeletion and $this->spyPickingListsScheduledForDeletion->contains($l)) {
                $this->spyPickingListsScheduledForDeletion->remove($this->spyPickingListsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param SpyPickingList $spyPickingList The SpyPickingList object to add.
     */
    protected function doAddSpyPickingList(SpyPickingList $spyPickingList): void
    {
        $this->collSpyPickingLists[]= $spyPickingList;
        $spyPickingList->setSpyStock($this);
    }

    /**
     * @param SpyPickingList $spyPickingList The SpyPickingList object to remove.
     * @return $this The current object (for fluent API support)
     */
    public function removeSpyPickingList(SpyPickingList $spyPickingList)
    {
        if ($this->getSpyPickingLists()->contains($spyPickingList)) {
            $pos = $this->collSpyPickingLists->search($spyPickingList);
            $this->collSpyPickingLists->remove($pos);
            if (null === $this->spyPickingListsScheduledForDeletion) {
                $this->spyPickingListsScheduledForDeletion = clone $this->collSpyPickingLists;
                $this->spyPickingListsScheduledForDeletion->clear();
            }
            $this->spyPickingListsScheduledForDeletion[]= clone $spyPickingList;
            $spyPickingList->setSpyStock(null);
        }
    ...
    /**
     * Clears out the collSpyWarehouseUserAssignments collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return $this
     * @see addSpyWarehouseUserAssignments()
     */
    public function clearSpyWarehouseUserAssignments()
    {
        $this->collSpyWarehouseUserAssignments = null; // important to set this to NULL since that means it is uninitialized

        return $this;
    }

    /**
     * Reset is the collSpyWarehouseUserAssignments collection loaded partially.
     *
     * @return void
     */
    public function resetPartialSpyWarehouseUserAssignments($v = true): void
    {
        $this->collSpyWarehouseUserAssignmentsPartial = $v;
    }

    /**
     * Initializes the collSpyWarehouseUserAssignments collection.
     *
     * By default this just sets the collSpyWarehouseUserAssignments collection to an empty array (like clearcollSpyWarehouseUserAssignments());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param bool $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initSpyWarehouseUserAssignments(bool $overrideExisting = true): void
    {
        if (null !== $this->collSpyWarehouseUserAssignments && !$overrideExisting) {
            return;
        }

        $collectionClassName = SpyWarehouseUserAssignmentTableMap::getTableMap()->getCollectionClassName();

        $this->collSpyWarehouseUserAssignments = new $collectionClassName;
        $this->collSpyWarehouseUserAssignments->setModel('\Orm\Zed\WarehouseUser\Persistence\SpyWarehouseUserAssignment');
    }

    /**
     * Gets an array of SpyWarehouseUserAssignment objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param ConnectionInterface $con optional connection object
     * @return ObjectCollection|SpyWarehouseUserAssignment[] List of SpyWarehouseUserAssignment objects
     * @phpstan-return ObjectCollection&\Traversable<SpyWarehouseUserAssignment> List of SpyWarehouseUserAssignment objects
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function getSpyWarehouseUserAssignments(?Criteria $criteria = null, ?ConnectionInterface $con = null)
    {
        $partial = $this->collSpyWarehouseUserAssignmentsPartial && !$this->isNew();
        if (null === $this->collSpyWarehouseUserAssignments || null !== $criteria || $partial) {
            if ($this->isNew()) {
                // return empty collection
                if (null === $this->collSpyWarehouseUserAssignments) {
                    $this->initSpyWarehouseUserAssignments();
                    ...
                    $collSpyWarehouseUserAssignments = new $collectionClassName;
                    $collSpyWarehouseUserAssignments->setModel('\Orm\Zed\WarehouseUser\Persistence\SpyWarehouseUserAssignment');

                    return $collSpyWarehouseUserAssignments;
                }
            } else {
                $collSpyWarehouseUserAssignments = SpyWarehouseUserAssignmentQuery::create(null, $criteria)
                    ->filterBySpyStock($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collSpyWarehouseUserAssignmentsPartial && count($collSpyWarehouseUserAssignments)) {
                        $this->initSpyWarehouseUserAssignments(false);

                        foreach ($collSpyWarehouseUserAssignments as $obj) {
                            if (false == $this->collSpyWarehouseUserAssignments->contains($obj)) {
                                $this->collSpyWarehouseUserAssignments->append($obj);
                            }
                        }

                        $this->collSpyWarehouseUserAssignmentsPartial = true;
                    }

                    return $collSpyWarehouseUserAssignments;
                }

                if ($partial && $this->collSpyWarehouseUserAssignments) {
                    foreach ($this->collSpyWarehouseUserAssignments as $obj) {
                        if ($obj->isNew()) {
                            $collSpyWarehouseUserAssignments[] = $obj;
                        }
                    }
                }

                $this->collSpyWarehouseUserAssignments = $collSpyWarehouseUserAssignments;
                $this->collSpyWarehouseUserAssignmentsPartial = false;
            }
        }

        return $this->collSpyWarehouseUserAssignments;
    }

    /**
     * Sets a collection of SpyWarehouseUserAssignment objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param Collection $spyWarehouseUserAssignments A Propel collection.
     * @param ConnectionInterface $con Optional connection object
     * @return $this The current object (for fluent API support)
     */
    public function setSpyWarehouseUserAssignments(Collection $spyWarehouseUserAssignments, ?ConnectionInterface $con = null)
    {
        /** @var SpyWarehouseUserAssignment[] $spyWarehouseUserAssignmentsToDelete */
        $spyWarehouseUserAssignmentsToDelete = $this->getSpyWarehouseUserAssignments(new Criteria(), $con)->diff($spyWarehouseUserAssignments);


        $this->spyWarehouseUserAssignmentsScheduledForDeletion = $spyWarehouseUserAssignmentsToDelete;

        foreach ($spyWarehouseUserAssignmentsToDelete as $spyWarehouseUserAssignmentRemoved) {
            $spyWarehouseUserAssignmentRemoved->setSpyStock(null);
        }

        $this->collSpyWarehouseUserAssignments = null;
        foreach ($spyWarehouseUserAssignments as $spyWarehouseUserAssignment) {
            $this->addSpyWarehouseUserAssignment($spyWarehouseUserAssignment);
        }

        $this->collSpyWarehouseUserAssignments = $spyWarehouseUserAssignments;
        $this->collSpyWarehouseUserAssignmentsPartial = false;
    ...
    /**
     * Returns the number of related BaseSpyWarehouseUserAssignment objects.
     *
     * @param Criteria $criteria
     * @param bool $distinct
     * @param ConnectionInterface $con
     * @return int Count of related BaseSpyWarehouseUserAssignment objects.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function countSpyWarehouseUserAssignments(?Criteria $criteria = null, bool $distinct = false, ?ConnectionInterface $con = null): int
    {
        $partial = $this->collSpyWarehouseUserAssignmentsPartial && !$this->isNew();
        if (null === $this->collSpyWarehouseUserAssignments || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collSpyWarehouseUserAssignments) {
                return 0;
            }

            if ($partial && !$criteria) {
                return count($this->getSpyWarehouseUserAssignments());
            }

            $query = SpyWarehouseUserAssignmentQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collSpyWarehouseUserAssignments);
    }

    /**
     * Method called to associate a SpyWarehouseUserAssignment object to this object
     * through the SpyWarehouseUserAssignment foreign key attribute.
     *
     * @param SpyWarehouseUserAssignment $l SpyWarehouseUserAssignment
     * @return $this The current object (for fluent API support)
     */
    public function addSpyWarehouseUserAssignment(SpyWarehouseUserAssignment $l)
    {
        if ($this->collSpyWarehouseUserAssignments === null) {
            $this->initSpyWarehouseUserAssignments();
            $this->collSpyWarehouseUserAssignmentsPartial = true;
        ...
        if (!$this->collSpyWarehouseUserAssignments->contains($l)) {
            $this->doAddSpyWarehouseUserAssignment($l);

            if ($this->spyWarehouseUserAssignmentsScheduledForDeletion and $this->spyWarehouseUserAssignmentsScheduledForDeletion->contains($l)) {
                $this->spyWarehouseUserAssignmentsScheduledForDeletion->remove($this->spyWarehouseUserAssignmentsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param SpyWarehouseUserAssignment $spyWarehouseUserAssignment The SpyWarehouseUserAssignment object to add.
     */
    protected function doAddSpyWarehouseUserAssignment(SpyWarehouseUserAssignment $spyWarehouseUserAssignment): void
    {
        $this->collSpyWarehouseUserAssignments[]= $spyWarehouseUserAssignment;
        $spyWarehouseUserAssignment->setSpyStock($this);
    }

    /**
     * @param SpyWarehouseUserAssignment $spyWarehouseUserAssignment The SpyWarehouseUserAssignment object to remove.
     * @return $this The current object (for fluent API support)
     */
    public function removeSpyWarehouseUserAssignment(SpyWarehouseUserAssignment $spyWarehouseUserAssignment)
    {
        if ($this->getSpyWarehouseUserAssignments()->contains($spyWarehouseUserAssignment)) {
            $pos = $this->collSpyWarehouseUserAssignments->search($spyWarehouseUserAssignment);
            $this->collSpyWarehouseUserAssignments->remove($pos);
            if (null === $this->spyWarehouseUserAssignmentsScheduledForDeletion) {
                $this->spyWarehouseUserAssignmentsScheduledForDeletion = clone $this->collSpyWarehouseUserAssignments;
                $this->spyWarehouseUserAssignmentsScheduledForDeletion->clear();
            }
            $this->spyWarehouseUserAssignmentsScheduledForDeletion[]= clone $spyWarehouseUserAssignment;
            $spyWarehouseUserAssignment->setSpyStock(null);
        }
    ...
    public function clear()
    {
        ...
        $this->is_active = null;
        $this->name = null;
        $this->picking_list_strategy = null;
        $this->uuid = null;
        $this->alreadyInSave = false;
        $this->clearAllReferences();
        $this->applyDefaultValues();
    ...
    {
        if ($deep) {
            if ($this->collSpyPickingLists) {
                foreach ($this->collSpyPickingLists as $o) {
            ...
            if ($this->collSpyWarehouseUserAssignments) {
                foreach ($this->collSpyWarehouseUserAssignments as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)
    ...
    protected function setGeneratedUuid()
    {
        ...
        $name = 'spy_stock' . '.' . $this->getIdStock();
