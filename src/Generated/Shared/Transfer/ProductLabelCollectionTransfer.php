 ...
 */
class ProductLabelCollectionTransfer extends AbstractTransfer
{
    /**
     * @var string
     */
    ...
    {
        $this->productLabels = new ArrayObject();

        foreach ($productLabels as $key => $value) {
            $args = [$value];

            if ($this->transferMetadata[static::PRODUCT_LABELS]['is_associative']) {
                $args = [$key, $value];
            }

            $this->addProductLabel(...$args);
        }
     ...
     */
    public function getProductLabels(): ArrayObject
    {
