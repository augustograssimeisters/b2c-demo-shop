<?php

/**
 * (c) Spryker Systems GmbH copyright protected
 */

namespace Generated\Shared\Transfer;

use Spryker\Shared\Kernel\Transfer\AbstractTransfer;

/**
 * !!! THIS FILE IS AUTO-GENERATED, EVERY CHANGE WILL BE LOST WITH THE NEXT RUN OF TRANSFER GENERATOR
 * !!! DO NOT CHANGE ANYTHING IN THIS FILE
 */
class FlysystemConfigAws3v3Transfer extends AbstractTransfer
{
    /**
     * @var string
     */
    public const REGION = 'region';

    /**
     * @deprecated Will be removed in next major.
     */
    public const VERSION = 'version';

    /**
     * @var string
     */
    public const BUCKET = 'bucket';

    /**
     * @var string
     */
    public const SECRET = 'secret';

    /**
     * @var string
     */
    public const KEY = 'key';

    /**
     * @var string
     */
    public const PATH = 'path';

    /**
     * @deprecated Will be removed in next major.
     */
    public const ROOT = 'root';

    /**
     * @var string
     */
    public const ENDPOINT = 'endpoint';

    /**
     * @var string
     */
    public const IS_STREAM_READS = 'isStreamReads';

    /**
     * @var string|null
     */
    protected $region;

    /**
     * @var string|null
     */
    protected $version;

    /**
     * @var string|null
     */
    protected $bucket;

    /**
     * @var string|null
     */
    protected $secret;

    /**
     * @var string|null
     */
    protected $key;

    /**
     * @var string|null
     */
    protected $path;

    /**
     * @var string|null
     */
    protected $root;

    /**
     * @var string|null
     */
    protected $endpoint;

    /**
     * @var bool|null
     */
    protected $isStreamReads;

    /**
     * @var array<string, string>
     */
    protected $transferPropertyNameMap = [
        'region' => 'region',
        'Region' => 'region',
        'version' => 'version',
        'Version' => 'version',
        'bucket' => 'bucket',
        'Bucket' => 'bucket',
        'secret' => 'secret',
        'Secret' => 'secret',
        'key' => 'key',
        'Key' => 'key',
        'path' => 'path',
        'Path' => 'path',
        'root' => 'root',
        'Root' => 'root',
        'endpoint' => 'endpoint',
        'Endpoint' => 'endpoint',
        'is_stream_reads' => 'isStreamReads',
        'isStreamReads' => 'isStreamReads',
        'IsStreamReads' => 'isStreamReads',
    ];

    /**
     * @var array<string, array<string, mixed>>
     */
    protected $transferMetadata = [
        self::REGION => [
            'type' => 'string',
            'type_shim' => null,
            'name_underscore' => 'region',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
        self::VERSION => [
            'type' => 'string',
            'type_shim' => null,
            'name_underscore' => 'version',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
        self::BUCKET => [
            'type' => 'string',
            'type_shim' => null,
            'name_underscore' => 'bucket',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
        self::SECRET => [
            'type' => 'string',
            'type_shim' => null,
            'name_underscore' => 'secret',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
        self::KEY => [
            'type' => 'string',
            'type_shim' => null,
            'name_underscore' => 'key',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
        self::PATH => [
            'type' => 'string',
            'type_shim' => null,
            'name_underscore' => 'path',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
        self::ROOT => [
            'type' => 'string',
            'type_shim' => null,
            'name_underscore' => 'root',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
        self::ENDPOINT => [
            'type' => 'string',
            'type_shim' => null,
            'name_underscore' => 'endpoint',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
        self::IS_STREAM_READS => [
            'type' => 'bool',
            'type_shim' => null,
            'name_underscore' => 'is_stream_reads',
            'is_collection' => false,
            'is_transfer' => false,
            'is_value_object' => false,
            'rest_request_parameter' => 'no',
            'is_associative' => false,
            'is_nullable' => false,
            'is_strict' => false,
        ],
    ];

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $region
     *
     * @return $this
     */
    public function setRegion($region)
    {
        $this->region = $region;
        $this->modifiedProperties[self::REGION] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @return string|null
     */
    public function getRegion()
    {
        return $this->region;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $region
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setRegionOrFail($region)
    {
        if ($region === null) {
            $this->throwNullValueException(static::REGION);
        }

        return $this->setRegion($region);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return string
     */
    public function getRegionOrFail()
    {
        if ($this->region === null) {
            $this->throwNullValueException(static::REGION);
        }

        return $this->region;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @return $this
     */
    public function requireRegion()
    {
        $this->assertPropertyIsSet(self::REGION);

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @deprecated Will be removed in next major.
     *
     * @param string|null $version
     *
     * @return $this
     */
    public function setVersion($version)
    {
        $this->version = $version;
        $this->modifiedProperties[self::VERSION] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @deprecated Will be removed in next major.
     *
     * @return string|null
     */
    public function getVersion()
    {
        return $this->version;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @deprecated Will be removed in next major.
     *
     * @param string|null $version
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setVersionOrFail($version)
    {
        if ($version === null) {
            $this->throwNullValueException(static::VERSION);
        }

        return $this->setVersion($version);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @deprecated Will be removed in next major.
     *
     * @return string
     */
    public function getVersionOrFail()
    {
        if ($this->version === null) {
            $this->throwNullValueException(static::VERSION);
        }

        return $this->version;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @deprecated Will be removed in next major.
     *
     * @return $this
     */
    public function requireVersion()
    {
        $this->assertPropertyIsSet(self::VERSION);

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $bucket
     *
     * @return $this
     */
    public function setBucket($bucket)
    {
        $this->bucket = $bucket;
        $this->modifiedProperties[self::BUCKET] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @return string|null
     */
    public function getBucket()
    {
        return $this->bucket;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $bucket
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setBucketOrFail($bucket)
    {
        if ($bucket === null) {
            $this->throwNullValueException(static::BUCKET);
        }

        return $this->setBucket($bucket);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return string
     */
    public function getBucketOrFail()
    {
        if ($this->bucket === null) {
            $this->throwNullValueException(static::BUCKET);
        }

        return $this->bucket;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @return $this
     */
    public function requireBucket()
    {
        $this->assertPropertyIsSet(self::BUCKET);

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $secret
     *
     * @return $this
     */
    public function setSecret($secret)
    {
        $this->secret = $secret;
        $this->modifiedProperties[self::SECRET] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @return string|null
     */
    public function getSecret()
    {
        return $this->secret;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $secret
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setSecretOrFail($secret)
    {
        if ($secret === null) {
            $this->throwNullValueException(static::SECRET);
        }

        return $this->setSecret($secret);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return string
     */
    public function getSecretOrFail()
    {
        if ($this->secret === null) {
            $this->throwNullValueException(static::SECRET);
        }

        return $this->secret;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @return $this
     */
    public function requireSecret()
    {
        $this->assertPropertyIsSet(self::SECRET);

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $key
     *
     * @return $this
     */
    public function setKey($key)
    {
        $this->key = $key;
        $this->modifiedProperties[self::KEY] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @return string|null
     */
    public function getKey()
    {
        return $this->key;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $key
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setKeyOrFail($key)
    {
        if ($key === null) {
            $this->throwNullValueException(static::KEY);
        }

        return $this->setKey($key);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return string
     */
    public function getKeyOrFail()
    {
        if ($this->key === null) {
            $this->throwNullValueException(static::KEY);
        }

        return $this->key;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @return $this
     */
    public function requireKey()
    {
        $this->assertPropertyIsSet(self::KEY);

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $path
     *
     * @return $this
     */
    public function setPath($path)
    {
        $this->path = $path;
        $this->modifiedProperties[self::PATH] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @return string|null
     */
    public function getPath()
    {
        return $this->path;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $path
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setPathOrFail($path)
    {
        if ($path === null) {
            $this->throwNullValueException(static::PATH);
        }

        return $this->setPath($path);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return string
     */
    public function getPathOrFail()
    {
        if ($this->path === null) {
            $this->throwNullValueException(static::PATH);
        }

        return $this->path;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @return $this
     */
    public function requirePath()
    {
        $this->assertPropertyIsSet(self::PATH);

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @deprecated Will be removed in next major.
     *
     * @param string|null $root
     *
     * @return $this
     */
    public function setRoot($root)
    {
        $this->root = $root;
        $this->modifiedProperties[self::ROOT] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @deprecated Will be removed in next major.
     *
     * @return string|null
     */
    public function getRoot()
    {
        return $this->root;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @deprecated Will be removed in next major.
     *
     * @param string|null $root
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setRootOrFail($root)
    {
        if ($root === null) {
            $this->throwNullValueException(static::ROOT);
        }

        return $this->setRoot($root);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @deprecated Will be removed in next major.
     *
     * @return string
     */
    public function getRootOrFail()
    {
        if ($this->root === null) {
            $this->throwNullValueException(static::ROOT);
        }

        return $this->root;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @deprecated Will be removed in next major.
     *
     * @return $this
     */
    public function requireRoot()
    {
        $this->assertPropertyIsSet(self::ROOT);

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $endpoint
     *
     * @return $this
     */
    public function setEndpoint($endpoint)
    {
        $this->endpoint = $endpoint;
        $this->modifiedProperties[self::ENDPOINT] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @return string|null
     */
    public function getEndpoint()
    {
        return $this->endpoint;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param string|null $endpoint
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setEndpointOrFail($endpoint)
    {
        if ($endpoint === null) {
            $this->throwNullValueException(static::ENDPOINT);
        }

        return $this->setEndpoint($endpoint);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return string
     */
    public function getEndpointOrFail()
    {
        if ($this->endpoint === null) {
            $this->throwNullValueException(static::ENDPOINT);
        }

        return $this->endpoint;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @return $this
     */
    public function requireEndpoint()
    {
        $this->assertPropertyIsSet(self::ENDPOINT);

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param bool|null $isStreamReads
     *
     * @return $this
     */
    public function setIsStreamReads($isStreamReads)
    {
        $this->isStreamReads = $isStreamReads;
        $this->modifiedProperties[self::IS_STREAM_READS] = true;

        return $this;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @return bool|null
     */
    public function getIsStreamReads()
    {
        return $this->isStreamReads;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @param bool|null $isStreamReads
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return $this
     */
    public function setIsStreamReadsOrFail($isStreamReads)
    {
        if ($isStreamReads === null) {
            $this->throwNullValueException(static::IS_STREAM_READS);
        }

        return $this->setIsStreamReads($isStreamReads);
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\NullValueException
     *
     * @return bool
     */
    public function getIsStreamReadsOrFail()
    {
        if ($this->isStreamReads === null) {
            $this->throwNullValueException(static::IS_STREAM_READS);
        }

        return $this->isStreamReads;
    }

    /**
     * @module FlysystemAws3v3FileSystem
     *
     * @throws \Spryker\Shared\Kernel\Transfer\Exception\RequiredTransferPropertyException
     *
     * @return $this
     */
    public function requireIsStreamReads()
    {
        $this->assertPropertyIsSet(self::IS_STREAM_READS);

        return $this;
    }

    /**
     * @param array<string, mixed> $data
     * @param bool $ignoreMissingProperty
     *
     * @throws \InvalidArgumentException
     *
     * @return $this
     */
    public function fromArray(array $data, $ignoreMissingProperty = false)
    {
        foreach ($data as $property => $value) {
            $normalizedPropertyName = $this->transferPropertyNameMap[$property] ?? null;

            switch ($normalizedPropertyName) {
                case 'region':
                case 'version':
                case 'bucket':
                case 'secret':
                case 'key':
                case 'path':
                case 'root':
                case 'endpoint':
                case 'isStreamReads':
                    $this->$normalizedPropertyName = $value;
                    $this->modifiedProperties[$normalizedPropertyName] = true;

                    break;
                default:
                    if (!$ignoreMissingProperty) {
                        throw new \InvalidArgumentException(sprintf('Missing property `%s` in `%s`', $property, static::class));
                    }
            }
        }

        return $this;
    }

    /**
     * @param bool $isRecursive
     * @param bool $camelCasedKeys
     *
     * @return array<string, mixed>
     */
    public function modifiedToArray($isRecursive = true, $camelCasedKeys = false): array
    {
        if ($isRecursive && !$camelCasedKeys) {
            return $this->modifiedToArrayRecursiveNotCamelCased();
        }
        if ($isRecursive && $camelCasedKeys) {
            return $this->modifiedToArrayRecursiveCamelCased();
        }
        if (!$isRecursive && $camelCasedKeys) {
            return $this->modifiedToArrayNotRecursiveCamelCased();
        }
        if (!$isRecursive && !$camelCasedKeys) {
            return $this->modifiedToArrayNotRecursiveNotCamelCased();
        }
    }

    /**
     * @param bool $isRecursive
     * @param bool $camelCasedKeys
     *
     * @return array<string, mixed>
     */
    public function toArray($isRecursive = true, $camelCasedKeys = false): array
    {
        if ($isRecursive && !$camelCasedKeys) {
            return $this->toArrayRecursiveNotCamelCased();
        }
        if ($isRecursive && $camelCasedKeys) {
            return $this->toArrayRecursiveCamelCased();
        }
        if (!$isRecursive && !$camelCasedKeys) {
            return $this->toArrayNotRecursiveNotCamelCased();
        }
        if (!$isRecursive && $camelCasedKeys) {
            return $this->toArrayNotRecursiveCamelCased();
        }
    }

    /**
     * @param array<string, mixed>|\ArrayObject<string, mixed> $value
     * @param bool $isRecursive
     * @param bool $camelCasedKeys
     *
     * @return array<string, mixed>
     */
    protected function addValuesToCollectionModified($value, $isRecursive, $camelCasedKeys): array
    {
        $result = [];
        foreach ($value as $elementKey => $arrayElement) {
            if ($arrayElement instanceof AbstractTransfer) {
                $result[$elementKey] = $arrayElement->modifiedToArray($isRecursive, $camelCasedKeys);

                continue;
            }
            $result[$elementKey] = $arrayElement;
        }

        return $result;
    }

    /**
     * @param array<string, mixed>|\ArrayObject<string, mixed> $value
     * @param bool $isRecursive
     * @param bool $camelCasedKeys
     *
     * @return array<string, mixed>
     */
    protected function addValuesToCollection($value, $isRecursive, $camelCasedKeys): array
    {
        $result = [];
        foreach ($value as $elementKey => $arrayElement) {
            if ($arrayElement instanceof AbstractTransfer) {
                $result[$elementKey] = $arrayElement->toArray($isRecursive, $camelCasedKeys);

                continue;
            }
            $result[$elementKey] = $arrayElement;
        }

        return $result;
    }

    /**
     * @return array<string, mixed>
     */
    public function modifiedToArrayRecursiveCamelCased(): array
    {
        $values = [];
        foreach ($this->modifiedProperties as $property => $_) {
            $value = $this->$property;

            $arrayKey = $property;

            if ($value instanceof AbstractTransfer) {
                $values[$arrayKey] = $value->modifiedToArray(true, true);

                continue;
            }
            switch ($property) {
                case 'region':
                case 'version':
                case 'bucket':
                case 'secret':
                case 'key':
                case 'path':
                case 'root':
                case 'endpoint':
                case 'isStreamReads':
                    $values[$arrayKey] = $value;

                    break;
            }
        }

        return $values;
    }

    /**
     * @return array<string, mixed>
     */
    public function modifiedToArrayRecursiveNotCamelCased(): array
    {
        $values = [];
        foreach ($this->modifiedProperties as $property => $_) {
            $value = $this->$property;

            $arrayKey = $this->transferMetadata[$property]['name_underscore'];

            if ($value instanceof AbstractTransfer) {
                $values[$arrayKey] = $value->modifiedToArray(true, false);

                continue;
            }
            switch ($property) {
                case 'region':
                case 'version':
                case 'bucket':
                case 'secret':
                case 'key':
                case 'path':
                case 'root':
                case 'endpoint':
                case 'isStreamReads':
                    $values[$arrayKey] = $value;

                    break;
            }
        }

        return $values;
    }

    /**
     * @return array<string, mixed>
     */
    public function modifiedToArrayNotRecursiveNotCamelCased(): array
    {
        $values = [];
        foreach ($this->modifiedProperties as $property => $_) {
            $value = $this->$property;

            $arrayKey = $this->transferMetadata[$property]['name_underscore'];

            $values[$arrayKey] = $value;
        }

        return $values;
    }

    /**
     * @return array<string, mixed>
     */
    public function modifiedToArrayNotRecursiveCamelCased(): array
    {
        $values = [];
        foreach ($this->modifiedProperties as $property => $_) {
            $value = $this->$property;

            $arrayKey = $property;

            $values[$arrayKey] = $value;
        }

        return $values;
    }

    /**
     * @return void
     */
    protected function initCollectionProperties(): void
    {
    }

    /**
     * @return array<string, mixed>
     */
    public function toArrayNotRecursiveCamelCased(): array
    {
        return [
            'region' => $this->region,
            'version' => $this->version,
            'bucket' => $this->bucket,
            'secret' => $this->secret,
            'key' => $this->key,
            'path' => $this->path,
            'root' => $this->root,
            'endpoint' => $this->endpoint,
            'isStreamReads' => $this->isStreamReads,
        ];
    }

    /**
     * @return array<string, mixed>
     */
    public function toArrayNotRecursiveNotCamelCased(): array
    {
        return [
            'region' => $this->region,
            'version' => $this->version,
            'bucket' => $this->bucket,
            'secret' => $this->secret,
            'key' => $this->key,
            'path' => $this->path,
            'root' => $this->root,
            'endpoint' => $this->endpoint,
            'is_stream_reads' => $this->isStreamReads,
        ];
    }

    /**
     * @return array<string, mixed>
     */
    public function toArrayRecursiveNotCamelCased(): array
    {
        return [
            'region' => $this->region instanceof AbstractTransfer ? $this->region->toArray(true, false) : $this->region,
            'version' => $this->version instanceof AbstractTransfer ? $this->version->toArray(true, false) : $this->version,
            'bucket' => $this->bucket instanceof AbstractTransfer ? $this->bucket->toArray(true, false) : $this->bucket,
            'secret' => $this->secret instanceof AbstractTransfer ? $this->secret->toArray(true, false) : $this->secret,
            'key' => $this->key instanceof AbstractTransfer ? $this->key->toArray(true, false) : $this->key,
            'path' => $this->path instanceof AbstractTransfer ? $this->path->toArray(true, false) : $this->path,
            'root' => $this->root instanceof AbstractTransfer ? $this->root->toArray(true, false) : $this->root,
            'endpoint' => $this->endpoint instanceof AbstractTransfer ? $this->endpoint->toArray(true, false) : $this->endpoint,
            'is_stream_reads' => $this->isStreamReads instanceof AbstractTransfer ? $this->isStreamReads->toArray(true, false) : $this->isStreamReads,
        ];
    }

    /**
     * @return array<string, mixed>
     */
    public function toArrayRecursiveCamelCased(): array
    {
        return [
            'region' => $this->region instanceof AbstractTransfer ? $this->region->toArray(true, true) : $this->region,
            'version' => $this->version instanceof AbstractTransfer ? $this->version->toArray(true, true) : $this->version,
            'bucket' => $this->bucket instanceof AbstractTransfer ? $this->bucket->toArray(true, true) : $this->bucket,
            'secret' => $this->secret instanceof AbstractTransfer ? $this->secret->toArray(true, true) : $this->secret,
            'key' => $this->key instanceof AbstractTransfer ? $this->key->toArray(true, true) : $this->key,
            'path' => $this->path instanceof AbstractTransfer ? $this->path->toArray(true, true) : $this->path,
            'root' => $this->root instanceof AbstractTransfer ? $this->root->toArray(true, true) : $this->root,
            'endpoint' => $this->endpoint instanceof AbstractTransfer ? $this->endpoint->toArray(true, true) : $this->endpoint,
            'isStreamReads' => $this->isStreamReads instanceof AbstractTransfer ? $this->isStreamReads->toArray(true, true) : $this->isStreamReads,
        ];
    }
}
