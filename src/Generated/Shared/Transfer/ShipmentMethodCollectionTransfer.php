 ...
 */
class ShipmentMethodCollectionTransfer extends AbstractTransfer
{
    /**
     * @var string
     */
    ...
    /**
     * @module ShipmentTypeStorage|ShipmentType|Shipment
     *
    ...
    {
        $this->shipmentMethods = new ArrayObject();

        foreach ($shipmentMethods as $key => $value) {
            $args = [$value];

            if ($this->transferMetadata[static::SHIPMENT_METHODS]['is_associative']) {
                $args = [$key, $value];
            }

            $this->addShipmentMethod(...$args);
        }
     ...
     */
    public function getShipmentMethods(): ArrayObject
    {
