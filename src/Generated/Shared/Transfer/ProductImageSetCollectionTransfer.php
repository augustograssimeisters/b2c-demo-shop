 ...
 */
class ProductImageSetCollectionTransfer extends AbstractTransfer
{
    /**
     * @var string
     */
    ...
    /**
     * @module ProductImageCartConnector|ProductImageSetsBackendApi|ProductImage
     *
    ...
    {
        $this->productImageSets = new ArrayObject();

        foreach ($productImageSets as $key => $value) {
            $args = [$value];

            if ($this->transferMetadata[static::PRODUCT_IMAGE_SETS]['is_associative']) {
                $args = [$key, $value];
            }

            $this->addProductImageSet(...$args);
        }
     ...
     */
    public function getProductImageSets(): ArrayObject
    {
